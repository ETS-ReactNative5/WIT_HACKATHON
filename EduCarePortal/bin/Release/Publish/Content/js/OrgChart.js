...
OrgChart.prototype.load = function (t) {
    ...
    for (var e = [], r = 0; r < this.config.nodes.length; r++) this.config.nodes[r].ParentID == t && e.push(JSON.parse(JSON.stringify(this.config.nodes[r])));
    return e
    ...
    if (this._canUpdateLink(t, e)) {
        ...
        if (r.isGroup && i.isGroup)
            for (var a = i.bgnodes[0].ID, o = 0; o < r.bgnodes.length; o++) {
                (n = this.get(r.bgnodes[o].ID)).ParentID = a, this.update(n, !0)
            } else if (!r.isGroup && i.isGroup) {
                a = i.bgnodes[0].ID;
                (n = this.get(t)).ParentID = a, this.update(n, !0)
            } else if (r.isGroup && !i.isGroup)
            for (o = 0; o < r.bgnodes.length; o++) {
                (n = this.get(r.bgnodes[o].ID)).ParentID = e, this.update(n, !0)
            } else if (!r.isGroup && !i.isGroup) {
                var n;
                (n = this.get(t)).ParentID = e, this.update(n, !0)
    ...
    for (var i = null, a = this.config.nodes.length - 1; 0 <= a; a--)
        if (this.config.nodes[a].ID == t) {
            i = this.config.nodes[a].ParentID, this.config.nodes.splice(a, 1);
            break
        } var o = this.getByParentId(t);
    for (a = o.length - 1; 0 <= a; a--) o[a].ParentID = i, this.update(o[a], e);
    return !0
}, OrgChart.prototype.addNode = function (t, e) {
    if (!1 === this.add(t, e)) return !1;
    var r = OrgChart.action.insert;
    null != t.ParentID && null != t.ParentID || (r = OrgChart.action.update);
    var i = this.getBGNode(t.ParentID),
        a = t.ParentID;
    ...
    if (a.isGroup || o.isGroup || a.isChildOfGroup || o.isChildOfGroup)
        if (a.isGroup || o.isGroup || !a.isChildOfGroup || o.isChildOfGroup)
            if (a.isGroup || o.isGroup || a.isChildOfGroup || !o.isChildOfGroup)
                if (!a.isGroup && !o.isGroup && a.isChildOfGroup && o.isChildOfGroup) a._groupParentNodeId != o._groupParentNodeId && (this._removeGroupIfHasOneNode(a._groupParentNodeId, a.ID), this.removeNodeTag(a.ID, a._groupParentNodeId, !0), this.addNodeTag(a.ID, o._groupParentNodeId, !0), this.link(a.ID, o.ParentID));
            ...
            else this.addNodeTag(a.ID, o._groupParentNodeId, !0), this.link(a.ID, o.ParentID);
        else {
    ...
    for (var e in this.nodes) {
        this.destroy, (0 != (i = this.nodes[e]).childrenIds.length || 0 == i.childrenIds.length && null == i.ParentID) && (i.isAssistant || "split" == i.templateName || g.push(i.ID))
    }
    ...
    for (var e in this.nodes) {
        var r = this.nodes[e];
        (0 != r.childrenIds.length || 0 == r.childrenIds.length && null == r.ParentID) && (r.isAssistant || "split" == r.templateName || g.push(r.ID))
    }
    ...
    this.templateName = i, this.ID = t, this.ParentID = e, this.x = null, this.y = null, this.level = null, this.leftNeighbor = null, this.rightNeighbor = null, this._prelim = 0, this._modifier = 0, this.children = [], this.parent = null, this.isAssistant = !1, this.tags = r, this.collapsedChildrenIds = [], this.childrenIds = [];
    ...
    OrgChart.remote._findRegion = function (r) {
        ...
        //        var e = "https://" + i[o] + "-/api/OrgChartJS",
        //            t = a[o];
    ...
    OrgChart.searchUI = function () { }, OrgChart.searchUI.prototype.init = function (t) {
        ...
        for (var i = a.nodes, o = {}, n = [], l = [], s = [], h = {}, d = !1, c = 0; c < i.length; c++) {
            ...
            if ((v = new OrgChart.node(g.ID, g.ParentID, p, u)).isAssistant = -1 != p.indexOf("assistant"), v.isAssistant && -1 == s.indexOf(v.ParentID) && OrgChart.server._isNotNullEmptyUndefined(v.ParentID) && s.push(v.ParentID), null != f) {
                if (!o[f.name]) {
                    n.push(f.name);
                    var C = new OrgChart.node(f.name, g.ParentID, [f.name], f.tag.template);
        ...
        if (null != a.orderBy && n.sort(function (t, e) {
            ...
            for (c = 0; c < n.length; c++) {
                h[(v = o[n[c]]).ParentID] && v.ID != h[v.ParentID] && (v.ParentID = h[v.ParentID])
            }
        var m = [];
        for (c = 0; c < n.length; c++) {
            var O = n[c],
                v = o[O],
                y = r ? r[O] : null;
            if (OrgChart.server._isNotNullEmptyUndefined(v.ParentID)) (H = o[v.ParentID]) ? ((v.parent = H).children.push(v), H.childrenIds.push(v.ID)) : (v.ParentID = null, v.parent = null, v.isAssistant = !1, l.push(v));
        ...
        for (c = 0; c < s.length; c++) {
            ...
            if (H) {
                ...
                for (b = H.children.length - 1; 0 <= b; b--) {
                    (z = H.children[b]).isAssistant ? (z.parent = null, H.children.splice(b, 1), H.childrenIds.splice(b, 1), k.splice(0, 0, z.ID)) : (z.ParentID = _.ID, (z.parent = _).children.unshift(z), _.childrenIds.unshift(z.ID), H.children.splice(b, 1))
                }
                if (k.length % 2) {
                    ...
                    OrgChart.templates[I] = {
                        ...
                        nodeMenuButton: "",
                        size: S
                    }, (W = new OrgChart.node(H.ID + "_transperant", A.ParentID, [], I)).isAssistant = !0, o[W.ID] = W, k.splice(k.length - 1, 0, W.ID)
                }
                var N = 1;
                ...
                for (b = 0; b < k.length; b += 3) {
                    ...
                    var E = o[k[b]],
                        L = o[k[b + 1]],
                        B = o[k[b + 2]];
                    E.parent = M, E.ParentID = M.ID, L.parent = M, L.ParentID = M.ID, B.parent = M, B.ParentID = M.ID, M.children.push(E), M.children.push(L), M.children.push(B), M.childrenIds.push(E.ID), M.childrenIds.push(L.ID), M.childrenIds.push(B.ID);
                    for (var T = M;
                    ...
                    if (H = v.parent) {
                        ...
                        for (b = H.children.length - 1; 0 <= b; b--) {
                            if ((z = H.children[b]) != v && 0 == z.childrenIds.length && !z.isAssistant && "split" != z.templateName) {
                                z.parent = v, z.ParentID = v.ID, z.layout = OrgChart.mixed, H.children.splice(b, 1);
        ...
        if (a.layout == OrgChart.tree || a.layout == OrgChart.treeRightOffset || a.layout == OrgChart.treeLeftOffset) {
            ...
            for (c = 0; c < q.length; c++) {
                ...
                for (b = 0; b < V.length; b += 3) {
                    ...
                    0 != b && (M = o[V[b - 3]]), 0 == b || L || (M = o[V[b - 2]]), E.parent = M, E.ParentID = M.ID, M.children.push(E), H.childrenIds.push(E.ID), L && (0 != b && (M = o[V[b - 2]]), L.parent = M, L.ParentID = M.ID, M.children.push(L), H.childrenIds.push(L.ID)), B && (0 != b && (M = o[V[b - 1]]), B.parent = M, B.ParentID = M.ID, M.children.push(B), H.childrenIds.push(B.ID))
                }
            }
        }
        return {
        ...
        if (i.collapse.level) {
            ...
            if (!i.roots || -1 == i.roots.indexOf(t.ID)) {
                for (var l = e[t.ParentID]; OrgChart.server._isSplit(l);) l = e[l.ParentID];
        ...
        if (r == OrgChart.action.expandCollapse || r == OrgChart.action.groupMaxMin || r == OrgChart.action.centerNode || r == OrgChart.action.update || r == OrgChart.action.insert) {
            ...
            if (null != i.ID) {
                var n = i.ID;
                r == OrgChart.action.groupMaxMin && e[n].ParentID && (n = e[n].ParentID);
                var l = e[n];
                if (!l)
                    for (var s in e)
